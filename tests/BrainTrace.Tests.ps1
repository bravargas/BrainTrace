...
Describe 'BrainTrace MVP configuration' {
    ...
    It 'keeps local cleanup and remote collection paths distinct' {
        ...
        $web.Logs[0].LocalPath|Should -BeExactly 'D:\Program File\Fiserv\Mobiliti\MTS Platform\Logs'
        $web.Logs[0].UNCPath|Should -BeExactly '\\vsmobwebdev05\d$\Program File\Fiserv\Mobiliti\MTS Platform\Logs'
    }
...
Describe 'BrainTrace DryRun safety' {
    It 'prints every DEV node without creating files' {
        ...
        $output=& (Join-Path $repo 'BrainTrace.ps1') Prepare -Environment DEV -DryRun 6>&1|Out-String
    ...
    It 'shows directional DEV collection executors and endpoints' {
        $output=& (Join-Path $repo 'BrainTrace.ps1') Collect -Environment DEV -Name Test -DryRun 6>&1|Out-String
        $output|Should -Match 'Executor: vsmobappdev03'
        $output|Should -Match ([regex]::Escape('\\vsmobwebdev05\d$\Program File\Fiserv\Mobiliti\MTS Platform\Logs'))
